...
use crate::most_beautiful_stats::{
    MostBeautifulGameAuditCardRecord, MostBeautifulGameAuditRoundRecord,
    MostBeautifulGameAuditScoreRecord, MostBeautifulGameAuditVoteRecord, MostBeautifulRoundRecord,
...
pub struct DixitEndRoundHistoryEntry {
    round_num: u16,
    storyteller: String,
    clue: String,
...
pub struct LeaderboardScoreSnapshot {
    total: u16,
    beauty: u16,
}

#[derive(Debug, Clone)]
struct CurrentLeaderboardScoreEntry {
    name: String,
    is_player: bool,
    has_score: bool,
}
...
struct RoomState {
    ...
    player_to_current_cards: HashMap<String, Vec<String>>,
    // private in-progress card-choice drafts synced by connected clients
    player_to_current_card_drafts: HashMap<String, Vec<String>>,
    // for each player, the card they voted for as being the active's card
    // they cannot vote for themselves; duplicates are allowed
    player_to_votes: HashMap<String, Vec<String>>,
    // private in-progress storyteller-vote drafts synced by connected clients
    player_to_vote_drafts: HashMap<String, Vec<String>>,
    ...
    // per voter, individual card tokens generated by forced random fill
    forced_random_vote_cards: HashMap<String, Vec<String>>,
    ...
    stella_player_selection_drafts: HashMap<String, Vec<String>>,
    ...
    deck_refill_count: u32,
    // room-wide default leaderboard mode for Dixit rooms
    leaderboard_view_mode_default: LeaderboardViewMode,
    // bump when moderators force-push a new leaderboard view mode
    leaderboard_view_mode_default_version: u64,
}
...
impl Room {
    pub fn new(
        ...
        let state = RoomState {
            ...
            player_to_current_card_drafts: HashMap::new(),
        ...
        let guessers = state
            ...
            .filter(|player| {
                active_player != Some(player.as_str())
                    && self.has_non_forced_vote_token(state, player)
    ...
    fn can_force_stella_associate_stage(&self, state: &RwLockWriteGuard<'_, RoomState>) -> bool {
        state
            ...
            .keys()
            .chain(state.stella_player_selection_drafts.keys())
            .any(|player| state.players.contains_key(player.as_str()))
    }
    ...
    fn fill_missing_stella_associate_selections(
        ...
        for player in state.player_order.clone() {
            ...
            let Some(draft) = state.stella_player_selection_drafts.get(&player).cloned() else {
                continue;
            };
            let selected = self.sanitize_stella_selection_cards(state, &draft);
    ...
    fn materialize_beauty_vote_drafts(&self, state: &mut RwLockWriteGuard<'_, RoomState>) {
        if !matches!(state.stage, RoomStage::BeautyVoting) {
            return;
        }

        for player in state.player_order.clone() {
            if !state.players.contains_key(&player)
                || state.player_to_beauty_votes.contains_key(&player)
            {
                continue;
            }
            let Some(draft) = state.player_to_beauty_vote_drafts.get(&player).cloned() else {
                continue;
            };
            let votes = self.sanitize_beauty_vote_cards(state, &player, &draft);
            if votes.is_empty() {
                continue;
            }
            state.player_to_beauty_votes.insert(player.clone(), votes);
            if let Some(info) = state.players.get_mut(&player) {
                info.ready = true;
            }
        }
    ...
    async fn force_current_stage(
        ...
        match state.stage {
            ...
            RoomStage::BeautyVoting => {
                self.materialize_beauty_vote_drafts(state);
    ...
    fn sanitize_vote_cards(
        ...
        let center_cards = self
            .get_center_cards(state)
            .into_iter()
            .collect::<HashSet<_>>();
    ...
    fn sanitize_beauty_vote_cards(
        ...
        for card in cards {
            if !center_cards.contains(card.as_str()) || disabled_cards.contains(card.as_str()) {
                continue;
            }
            if !state.beauty_allow_duplicate_votes && !seen.insert(card.clone()) {
                continue;
            }
    ...
    fn has_non_forced_vote_token(
        ...
        let Some(votes) = state.player_to_votes.get(player) else {
            return false;
        };
        let forced = state
            .forced_random_vote_cards
            .get(player)
            .cloned()
            .unwrap_or_default();
        let mut forced_counts = HashMap::<String, usize>::new();
        for card in forced {
            *forced_counts.entry(card).or_insert(0) += 1;
        }
        votes.iter().any(|card| match forced_counts.get_mut(card) {
    ...
    fn has_non_forced_correct_vote(
        &self,
        state: &RwLockWriteGuard<'_, RoomState>,
        player: &str,
        active_card: &str,
        ...
        votes.iter().any(|card| {
            let is_forced_random_token = match forced_counts.get_mut(card) {
                Some(count) if *count > 0 => {
                    *count -= 1;
                    true
                }
            ...
            !is_forced_random_token && card == active_card
        })
    }
    ...
    fn compute_beauty_point_change_winner_bonus(
        ...
        let points_per_winner = if state.beauty_split_points_on_tie && winning_owners.len() > 1 {
            let winner_count = u16::try_from(winning_owners.len())
                .unwrap_or(u16::MAX)
                .max(1);
            state.beauty_points_bonus.saturating_add(winner_count - 1) / winner_count
        } else {
            state.beauty_points_bonus
        };
    ...
    fn member_hash_for_name(
        &self,
        state: &RwLockWriteGuard<'_, RoomState>,
        member_name: &str,
    ) -> String {
        if let Some(token) = state.name_tokens.get(member_name) {
            let mut hasher = Sha256::new();
            hasher.update(token.as_bytes());
        ...
        let mut hasher = Sha256::new();
        hasher.update(state.room_id.as_bytes());
        hasher.update([0]);
    ...
    fn build_audit_vote_records(
        ...
        state: &RwLockWriteGuard<'_, RoomState>,
        player_to_votes: &HashMap<String, Vec<String>>,
    ) -> Vec<MostBeautifulGameAuditVoteRecord> {
        let mut aggregated_votes: HashMap<(String, String), u16> = HashMap::new();
        for (voter_name, cards) in player_to_votes {
            for card_hash in cards {
                *aggregated_votes
                    .entry((voter_name.clone(), card_hash.clone()))
                    .or_insert(0) += 1;
            }
        }

        let mut records = aggregated_votes
            .into_iter()
            .map(
                |((voter_name, card_hash), vote_count)| MostBeautifulGameAuditVoteRecord {
                    voter_hash: self.member_hash_for_name(state, &voter_name),
                    voter_display_name: voter_name,
                    card_hash,
                    vote_count,
                },
            ...
            .collect::<Vec<_>>();
        records.sort_by(|a, b| {
            a.voter_display_name
                .cmp(&b.voter_display_name)
                .then(a.card_hash.cmp(&b.card_hash))
        });
        records
    }

    fn record_current_dixit_round_audit_and_history(
        &self,
        state: &mut RwLockWriteGuard<'_, RoomState>,
    ) -> Result<()> {
        ...
        let Some(game_id) = state.current_dixit_game_id.clone() else {
            return Ok(());
        };
        let Some(game_started_at_s) = state.current_dixit_game_started_at_s else {
            return Ok(());
        };
        let Some(storyteller_name) = state.player_order.get(state.active_player).cloned() else {
            return Ok(());
        };

        let storyteller_hash = self.member_hash_for_name(state, &storyteller_name);
        let card_to_owner = state
            .player_to_current_cards
            .iter()
            .flat_map(|(owner, cards)| cards.iter().map(|card| (card.clone(), owner.clone())))
        ...
        let storyteller_cards = state
            .player_to_current_cards
            .get(&storyteller_name)
            .cloned()
            .unwrap_or_default()
            .into_iter()
            .collect::<HashSet<_>>();
        let card_entries = center_cards
            .iter()
            .enumerate()
            .filter_map(|(index, card_hash)| {
                let owner_name = card_to_owner.get(card_hash)?.clone();
                let owner_hash = self.member_hash_for_name(state, &owner_name);
                Some(MostBeautifulGameAuditCardRecord {
                    card_hash: card_hash.clone(),
                    owner_hash: owner_hash.clone(),
                    owner_display_name: owner_name.clone(),
                    submitted_by_hash: owner_hash,
                    submitted_by_display_name: owner_name,
                    is_storyteller_card: storyteller_cards.contains(card_hash),
                    center_order: u16::try_from(index).unwrap_or(u16::MAX),
                })
            })
            .collect::<Vec<_>>();
        let story_votes = self.build_audit_vote_records(state, &state.player_to_votes);
        let beauty_votes = self.build_audit_vote_records(state, &state.player_to_beauty_votes);
        let combined_point_change =
            self.merge_point_changes(&state.storyteller_point_change, &state.beauty_point_change);

        let mut score_member_names = state
            .players
            .keys()
            .chain(state.observers.keys())
            .cloned()
            .collect::<Vec<_>>();
        score_member_names.sort();

        let mut total_after_round = HashMap::new();
        let mut beauty_total_after_round = HashMap::new();
        let mut score_log = Vec::new();

        for member_name in score_member_names {
            let current_total = state
                ...
                .unwrap_or(0);
            let story_delta = state
                .storyteller_point_change
                .get(&member_name)
                .copied()
                .unwrap_or(0);
            let beauty_delta = state
                .beauty_point_change
                .get(&member_name)
                .copied()
                .unwrap_or(0);
            ...
            score_log.push(MostBeautifulGameAuditScoreRecord {
                player_hash: self.member_hash_for_name(state, &member_name),
                player_display_name: member_name,
                story_delta,
                beauty_delta,
                total_after_round: total_after,
                beauty_total_after_round: beauty_after,
            });
        }

        self.most_beautiful_stats
            .record_game_audit_round(&MostBeautifulGameAuditRoundRecord {
                game_id,
                room_id: state.room_id.clone(),
                game_started_at_s,
                recorded_at_s: get_time_s(),
                round_num: state.round,
                storyteller_hash,
                storyteller_display_name: storyteller_name.clone(),
                clue: state.current_description.clone(),
                results_display_mode: Self::beauty_results_display_mode_label(
                    state.beauty_results_display_mode,
                )
                .to_string(),
                card_entries,
    ...
    fn start_new_dixit_game_tracking(&self, state: &mut RwLockWriteGuard<'_, RoomState>) {
        let mut hasher = Sha256::new();
        hasher.update(state.room_id.as_bytes());
        hasher.update(get_time_ms().to_le_bytes());
    ...
    fn finalize_current_dixit_game(
        ...
        if let Some(game_id) = state.current_dixit_game_id.clone() {
            self.most_beautiful_stats
                .mark_game_complete(&game_id, get_time_s(), state.round)?;
        }
        Ok(())
    }

    fn transition_to_end(&self, state: &mut RwLockWriteGuard<'_, RoomState>) -> Result<()> {
    ...
    fn current_leaderboard_score_entries(
        state: &RwLockWriteGuard<'_, RoomState>,
    ) -> Vec<CurrentLeaderboardScoreEntry> {
        state
            .players
            .iter()
            .map(|(name, _)| CurrentLeaderboardScoreEntry {
                name: name.clone(),
                is_player: true,
                has_score: true,
            })
            .chain(
                state
                    .observers
                    .iter()
                    .map(|(name, info)| CurrentLeaderboardScoreEntry {
                        name: name.clone(),
                        is_player: false,
                        has_score: info.points.is_some(),
                    }),
            )
            .collect()
    }

    fn leaderboard_since_joined_scores_by_round(
        &self,
        state: &RwLockWriteGuard<'_, RoomState>,
    ) -> HashMap<u16, HashMap<String, LeaderboardScoreSnapshot>> {
    ...
    fn since_joined_score_map(
        ...
        for round in sorted_history
            .iter()
            .filter(|round| round.round_num >= start_round)
        {
            let participants = Self::leaderboard_round_participants(round);
            let simulated_floor =
                Self::simulated_floor_for_round(&round.active_players, &simulated_scores);

            for participant in &participants {
                if snapshot_offsets.contains_key(participant) {
                    continue;
                }

                let starts_as_active_player =
                    round.active_players.iter().any(|name| name == participant);
                if !starts_as_active_player && first_active_rounds.contains_key(participant) {
                    continue;
                }

                let initial_total = if starts_as_active_player {
                    simulated_floor
                } else {
                    0
                };
                let simulated_total_after_first_round = Self::apply_signed_delta(
                    initial_total,
                    round.total_deltas.get(participant).copied().unwrap_or(0),
                );
                let simulated_beauty_after_first_round = Self::apply_signed_delta(
                    0,
                    round.beauty_deltas.get(participant).copied().unwrap_or(0),
                );

                snapshot_offsets.insert(
                    participant.clone(),
                    (
                        Self::recorded_total_after_round(round, participant)
                            - i32::from(simulated_total_after_first_round),
                        Self::recorded_beauty_after_round(round, participant)
                            - i32::from(simulated_beauty_after_first_round),
                    ),
                );
            }

            for participant in participants {
                let Some((total_offset, beauty_offset)) = snapshot_offsets.get(&participant) else {
                    continue;
                };
                simulated_scores.insert(
                    participant.clone(),
                    LeaderboardScoreSnapshot {
                        total: Self::apply_snapshot_offset(
                            Self::recorded_total_after_round(round, &participant),
                            *total_offset,
                        ),
                        beauty: Self::apply_snapshot_offset(
                            Self::recorded_beauty_after_round(round, &participant),
                            *beauty_offset,
                        ),
                    },
        ...
        let live_floor = Self::live_simulated_floor(entries, &simulated_scores);
        for entry in entries {
            if !entry.has_score || simulated_scores.contains_key(&entry.name) {
                continue;
            }
            simulated_scores.insert(
                entry.name.clone(),
                LeaderboardScoreSnapshot {
                    total: if entry.is_player { live_floor } else { 0 },
                    beauty: 0,
                },
            );
        }

        let current_scored_entry_names = entries
            .iter()
            .filter(|entry| entry.has_score)
            .map(|entry| entry.name.clone())
            .collect::<HashSet<_>>();
        simulated_scores.retain(|name, _| current_scored_entry_names.contains(name));
        ...
        for entry in entries {
            ...
            simulated_scores
                ...
                .or_insert(LeaderboardScoreSnapshot {
                    total: 0,
                    beauty: 0,
                });
        }
    ...
    fn sorted_leaderboard_history(
        history: &[LeaderboardRoundHistoryEntry],
    ) -> Vec<&LeaderboardRoundHistoryEntry> {
        let mut sorted_history = history.iter().collect::<Vec<_>>();
        sorted_history.sort_by_key(|round| round.round_num);
        sorted_history
    }
    ...
    fn first_active_rounds_at_or_after(
        ...
        for round in history
            ...
            .filter(|round| round.round_num >= start_round)
        {
            for player_name in &round.active_players {
                first_active_rounds
    ...
    fn leaderboard_round_participants(round: &LeaderboardRoundHistoryEntry) -> Vec<String> {
        let mut participants = round
            .active_players
            .iter()
            .chain(round.total_after_round.keys())
            .chain(round.beauty_total_after_round.keys())
            .chain(round.total_deltas.keys())
            .chain(round.beauty_deltas.keys())
            .cloned()
            .collect::<Vec<_>>();
        participants.sort();
        participants.dedup();
    ...
    fn recorded_total_after_round(round: &LeaderboardRoundHistoryEntry, participant: &str) -> i32 {
        round
            .total_after_round
            .get(participant)
            .map(|value| i32::from(*value))
            .or_else(|| round.total_deltas.get(participant).copied())
            .unwrap_or(0)
    }

    fn recorded_beauty_after_round(round: &LeaderboardRoundHistoryEntry, participant: &str) -> i32 {
        round
            .beauty_total_after_round
            .get(participant)
            .map(|value| i32::from(*value))
            .or_else(|| round.beauty_deltas.get(participant).copied())
            .unwrap_or(0)
    }

    fn apply_snapshot_offset(recorded: i32, offset: i32) -> u16 {
        let adjusted = recorded.saturating_sub(offset);
        if adjusted <= 0 {
            0
        } else {
            adjusted.min(i32::from(u16::MAX)) as u16
        }
    }

    fn simulated_floor_for_round(
        active_players: &[String],
        scores: &HashMap<String, LeaderboardScoreSnapshot>,
    ) -> u16 {
        active_players
            .iter()
            .filter_map(|player_name| scores.get(player_name).map(|score| score.total))
            .min()
            .unwrap_or(0)
    }

    fn live_simulated_floor(
        entries: &[CurrentLeaderboardScoreEntry],
        scores: &HashMap<String, LeaderboardScoreSnapshot>,
    ) -> u16 {
        entries
            ...
            .filter_map(|entry| scores.get(&entry.name).map(|score| score.total))
            .min()
            .unwrap_or(0)
    }
    ...
    async fn submit_votes(
        ...
        state.player_to_vote_drafts.remove(name);
        if let Some(player) = state.players.get_mut(name) {
            player.ready = true;
        }
        self.broadcast_msg(self.room_state(state))?;
    ...
    fn set_player_choose_draft(
        &self,
        state: &mut RwLockWriteGuard<'_, RoomState>,
        name: &str,
        cards: Vec<String>,
    ) {
        if !state.players.contains_key(name) || !matches!(state.stage, RoomStage::PlayersChoose) {
            return;
        }
        let cards = self.sanitize_player_choose_cards(state, name, &cards);
        if cards.is_empty() {
            state.player_to_current_card_drafts.remove(name);
        } else {
            state
    ...
    fn set_vote_draft(
        ...
        let cards = self.sanitize_vote_cards(state, name, &cards);
        if cards.is_empty() {
            state.player_to_vote_drafts.remove(name);
        } else {
            state.player_to_vote_drafts.insert(name.to_string(), cards);
        }
    }

    fn set_beauty_vote_draft(
        &self,
        state: &mut RwLockWriteGuard<'_, RoomState>,
        name: &str,
        cards: Vec<String>,
    ) {
        if !state.players.contains_key(name) || !matches!(state.stage, RoomStage::BeautyVoting) {
            return;
        }
        let cards = self.sanitize_beauty_vote_cards(state, name, &cards);
        if cards.is_empty() {
            state.player_to_beauty_vote_drafts.remove(name);
        } else {
            state
    ...
    fn set_stella_selection_draft(
        &self,
        state: &mut RwLockWriteGuard<'_, RoomState>,
        name: &str,
        cards: Vec<String>,
    ) {
        if !state.players.contains_key(name) || !matches!(state.stage, RoomStage::StellaAssociate) {
            return;
        }
        let cards = self.sanitize_stella_selection_cards(state, &cards);
        if cards.is_empty() {
            state.stella_player_selection_drafts.remove(name);
        } else {
            state
                .stella_player_selection_drafts
                .insert(name.to_string(), cards);
        }
    }
    ...
    async fn init_voting(&self, state: &mut RwLockWriteGuard<'_, RoomState>) -> Result<()> {
        ...
        for player in state.player_order.clone().iter() {
            ...
            let mut chosen = state
                ...
                .unwrap_or_default();
            if chosen.is_empty() {
                if let Some(draft) = state.player_to_current_card_drafts.get(player) {
                    chosen = self.sanitize_player_choose_cards(state, player, draft);
                }
            }
    ...
    fn autofill_missing_storyteller_votes(
        ...
        for player in state.player_order.clone().iter() {
            ...
            if votes.is_empty() && !draft_votes.is_empty() {
                *votes = draft_votes;
            }
    ...
    fn choose_stella_clue(
        ...
        candidates
            ...
            .or_else(|| self.default_stella_word_pack.first().cloned())
    ...
    pub async fn handle_client_msg(
        ...
        match msg {
            ...
            ClientMsg::SetBeautySplitPointsOnTie { enabled } => {
                if !self.is_moderator(&state, name) {
                    if let Some(tx) = state.player_to_socket.get(name) {
                        tx.send(
                            ServerMsg::ErrorMsg(
                ...
                state.beauty_split_points_on_tie = enabled;
                self.broadcast_msg(self.room_state(&state))?;
            }
            ...
            ClientMsg::SetLeaderboardViewModeDefault { mode } => {
                if !self.is_moderator(&state, name) {
                    if let Some(tx) = state.player_to_socket.get(name) {
                        tx.send(
                            ServerMsg::ErrorMsg(
                                "Only moderators can change leaderboard view defaults".to_string(),
                            )
                            .into(),
                        )
                        .await?;
                ...
                state.leaderboard_view_mode_default = mode;
                state.leaderboard_view_mode_default_version = state
                    .leaderboard_view_mode_default_version
                    .saturating_add(1);
                self.broadcast_msg(self.room_state(&state))?;
            }
            ...
            ClientMsg::SetPlayerChooseDraft { cards } => {
                self.set_player_choose_draft(&mut state, name, cards);
            }
            ClientMsg::SetVoteDraft { cards } => {
                self.set_vote_draft(&mut state, name, cards);
            }
            ClientMsg::SetBeautyVoteDraft { cards } => {
                self.set_beauty_vote_draft(&mut state, name, cards);
            }
            ClientMsg::SetStellaSelectionDraft { cards } => {
                self.set_stella_selection_draft(&mut state, name, cards);
            }
    ...
    fn compute_results(&self, state: &RwLockWriteGuard<RoomState>) -> HashMap<String, u16> {
        ...
        for (player, votes) in state.player_to_votes.iter() {
            ...
            let mut correct_vote_count = 0usize;
            for card in votes.iter() {
                ...
                if !is_forced_random_token {
                    *votes_for_card.entry(card.to_string()).or_insert(0) += 1;
                    if card == &active_card {
                        correct_vote_count += 1;
                    }
                }
            }
        ...
        if storyteller_loses {
            for player in round_participants.iter() {
                let has_correct_vote =
                    self.has_non_forced_correct_vote(state, player, &active_card);
    ...
    fn room_state(&self, state: &RwLockWriteGuard<RoomState>) -> ServerMsg {
        ...
        ServerMsg::RoomState {
            ...
            dixit_end_round_history: if matches!(state.stage, RoomStage::End)
                && matches!(state.game_mode, GameMode::DixitPlus)
            {
                state.dixit_end_round_history.clone()
            } else {
                Vec::new()
            },
            leaderboard_round_history: self.leaderboard_round_history(state),
            leaderboard_since_joined_scores_by_round: self
                .leaderboard_since_joined_scores_by_round(state),
...
mod tests {
    ...
    #[tokio::test]
    async fn beauty_point_change_splits_bonus_across_tied_owners_with_ceiling() -> Result<()> {
        let room = test_room();
        let mut state = room.state.write().await;
        ...
        assert_eq!(
            point_change.get("b").copied(),
            Some(1),
            "multiple tied winning cards should still count as one tied owner"
        );
        assert_eq!(
            point_change.get("c").copied(),
            Some(1),
            "tied owners should receive ceil(bonus / owner_count)"
        );
        assert_eq!(
            point_change.get("d").copied(),
            Some(1),
    ...
    async fn room_state_includes_threshold_correct_bonus_toggle_defaults() -> Result<()> {
        ...
        match room.room_state(&state) {
            ServerMsg::RoomState {
                ...
                assert!(
                    beauty_split_points_on_tie,
                    "beauty tie splitting should default to enabled"
                );
                assert!(
                ...
                assert_eq!(
                    leaderboard_view_mode_default_version, 0,
                    "leaderboard view default version should start at 0"
                );
                assert_eq!(
                    usize::from(stella_word_pack_size),
                    test_default_stella_word_pack().len(),
                    "room state should expose the full default Resonance word-pack size"
                );
                assert_eq!(
                    stella_word_pack_words,
                    (*test_default_stella_word_pack()).clone(),
                    "room state should expose the active default Resonance word pack"
                );
            }
...
alpha
  gamma  ",
    ...
    #[tokio::test]
    async fn stella_mode_ignores_beauty_timer_updates_and_leaderboard_view_pushes() -> Result<()> {
        let room = test_room();
        {
            let mut state = room.state.write().await;
            add_player(&mut state, "host", 0);
        ...
        room.handle_client_msg(
            "host",
            10_202,
            to_ws(ClientMsg::SetLeaderboardViewModeDefault {
                mode: LeaderboardViewMode::Combined,
            }),
        )
        .await?;
        ...
        assert_eq!(
            state.beauty_timer_duration_s, 60,
            "Stella should ignore Most Beautiful timer changes"
        );
        assert!(
            matches!(
    ...
    #[tokio::test]
    async fn moderator_can_force_push_leaderboard_view_mode_default() -> Result<()> {
        let room = test_room();
        {
            let mut state = room.state.write().await;
            add_player(&mut state, "host", 0);
        ...
        assert!(
            ...
            "moderator update should change the room-wide leaderboard view mode"
        );
        assert_eq!(
        ...
        match room.room_state(&state) {
            ServerMsg::RoomState {
                ...
                assert_eq!(
                    leaderboard_since_joined_scores_by_round
                        .get(&1)
                        .and_then(|scores| scores.get("a")),
                    Some(&LeaderboardScoreSnapshot {
                        total: 3,
                        beauty: 0
                    }),
                    "room state should expose server-computed since-joined score maps"
                );
            }
            _ => return Err(anyhow!("Expected RoomState")),
        }
    ...
    #[test]
    fn since_joined_round_one_projection_matches_raw_scores_for_later_joiners() {
        let history = vec![
            LeaderboardRoundHistoryEntry {
                round_num: 1,
                active_players: vec!["Alice".into()],
                total_deltas: HashMap::from([("Alice".into(), 10)]),
                beauty_deltas: HashMap::new(),
                total_after_round: HashMap::from([("Alice".into(), 10)]),
                beauty_total_after_round: HashMap::new(),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 2,
                active_players: vec!["Alice".into()],
                total_deltas: HashMap::from([("Alice".into(), 5)]),
                beauty_deltas: HashMap::new(),
                total_after_round: HashMap::from([("Alice".into(), 15), ("Bob".into(), 0)]),
                beauty_total_after_round: HashMap::new(),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 3,
                active_players: vec!["Alice".into(), "Bob".into()],
                total_deltas: HashMap::from([("Bob".into(), 2)]),
                beauty_deltas: HashMap::new(),
                total_after_round: HashMap::from([("Alice".into(), 15), ("Bob".into(), 17)]),
                beauty_total_after_round: HashMap::new(),
            },
        ];
        let entries = vec![
    ...
    #[test]
    fn since_joined_round_one_projection_matches_raw_beauty_scores() {
        let history = vec![
            LeaderboardRoundHistoryEntry {
                round_num: 1,
                active_players: vec!["Alice".into()],
                total_deltas: HashMap::from([("Alice".into(), 8)]),
                beauty_deltas: HashMap::from([("Alice".into(), 2)]),
                total_after_round: HashMap::from([("Alice".into(), 8)]),
                beauty_total_after_round: HashMap::from([("Alice".into(), 2)]),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 2,
                active_players: vec!["Alice".into()],
                total_deltas: HashMap::from([("Alice".into(), 4)]),
                beauty_deltas: HashMap::from([("Alice".into(), 1)]),
                total_after_round: HashMap::from([("Alice".into(), 12), ("Bob".into(), 0)]),
                beauty_total_after_round: HashMap::from([("Alice".into(), 3)]),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 3,
                active_players: vec!["Alice".into(), "Bob".into()],
                total_deltas: HashMap::from([("Alice".into(), 1), ("Bob".into(), 3)]),
                beauty_deltas: HashMap::from([("Bob".into(), 2)]),
                total_after_round: HashMap::from([("Alice".into(), 13), ("Bob".into(), 15)]),
                beauty_total_after_round: HashMap::from([("Alice".into(), 3), ("Bob".into(), 2)]),
            },
        ];
        let entries = vec![
            CurrentLeaderboardScoreEntry {
    ...
    #[test]
    fn since_joined_cutoff_offsets_preserve_beauty_and_story_breakdowns() {
        let history = vec![
            LeaderboardRoundHistoryEntry {
                round_num: 1,
                active_players: vec!["Alice".into()],
                total_deltas: HashMap::from([("Alice".into(), 10)]),
                beauty_deltas: HashMap::from([("Alice".into(), 4)]),
                total_after_round: HashMap::from([("Alice".into(), 10)]),
                beauty_total_after_round: HashMap::from([("Alice".into(), 4)]),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 2,
                active_players: vec!["Alice".into(), "Bob".into()],
                total_deltas: HashMap::from([("Bob".into(), 5)]),
                beauty_deltas: HashMap::from([("Bob".into(), 3)]),
                total_after_round: HashMap::from([("Alice".into(), 10), ("Bob".into(), 15)]),
                beauty_total_after_round: HashMap::from([("Alice".into(), 4), ("Bob".into(), 3)]),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 3,
                active_players: vec!["Alice".into(), "Bob".into()],
                total_deltas: HashMap::from([("Bob".into(), 2)]),
                beauty_deltas: HashMap::from([("Bob".into(), 1)]),
                total_after_round: HashMap::from([("Alice".into(), 10), ("Bob".into(), 17)]),
                beauty_total_after_round: HashMap::from([("Alice".into(), 4), ("Bob".into(), 4)]),
            },
        ];
        let entries = vec![
            CurrentLeaderboardScoreEntry {
        ...
        assert_eq!(
            ...
            "Bob's story-only total is total minus beauty, i.e. 3"
        );
    }

    #[test]
    fn since_joined_scored_observers_keep_their_own_delta_history() {
        let history = vec![
            LeaderboardRoundHistoryEntry {
                round_num: 1,
                active_players: vec!["Alice".into()],
            ...
            LeaderboardRoundHistoryEntry {
                round_num: 2,
                active_players: vec!["Alice".into()],
                total_deltas: HashMap::from([("Observer".into(), 4)]),
                beauty_deltas: HashMap::new(),
                total_after_round: HashMap::from([("Alice".into(), 10), ("Observer".into(), 4)]),
                beauty_total_after_round: HashMap::new(),
            },
        ];
        let entries = vec![
    ...
    #[test]
    fn since_joined_later_active_joiner_gets_simulated_floor() {
        let history = vec![
            LeaderboardRoundHistoryEntry {
                round_num: 1,
            ...
            LeaderboardRoundHistoryEntry {
                ...
                total_deltas: HashMap::from([("Existing".into(), 10)]),
                beauty_deltas: HashMap::new(),
                total_after_round: HashMap::from([("Existing".into(), 20)]),
                beauty_total_after_round: HashMap::new(),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 3,
                active_players: vec!["Existing".into(), "Bob".into()],
                total_deltas: HashMap::from([("Existing".into(), 5), ("Bob".into(), 2)]),
                beauty_deltas: HashMap::new(),
                total_after_round: HashMap::from([("Existing".into(), 25), ("Bob".into(), 22)]),
                beauty_total_after_round: HashMap::new(),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 4,
                active_players: vec!["Existing".into(), "Bob".into()],
                total_deltas: HashMap::from([("Existing".into(), 3), ("Bob".into(), 4)]),
                beauty_deltas: HashMap::new(),
                total_after_round: HashMap::from([("Existing".into(), 28), ("Bob".into(), 26)]),
                beauty_total_after_round: HashMap::new(),
            },
            LeaderboardRoundHistoryEntry {
                round_num: 5,
                active_players: vec!["Existing".into(), "Bob".into(), "Alice".into()],
                total_deltas: HashMap::from([
                    ("Existing".into(), 1),
                    ("Bob".into(), 1),
                    ("Alice".into(), 3),
                ]),
    ...
    #[test]
    fn since_joined_later_pending_observer_gets_simulated_floor_when_active() {
        let history = vec![
            LeaderboardRoundHistoryEntry {
                round_num: 1,
                active_players: vec!["Existing".into()],
            ...
            LeaderboardRoundHistoryEntry {
                round_num: 5,
                active_players: vec!["Existing".into(), "Bob".into()],
                total_deltas: HashMap::from([("Existing".into(), 1), ("Bob".into(), 1)]),
                beauty_deltas: HashMap::new(),
                total_after_round: HashMap::from([
                    ("Existing".into(), 29),
                    ("Bob".into(), 27),
    ...
    #[test]
    fn since_joined_live_active_joiner_gets_current_simulated_floor() {
        let history = vec![
            LeaderboardRoundHistoryEntry {
                round_num: 1,
                active_players: vec!["Existing".into()],
    ...
    #[test]
    fn since_joined_pending_observer_without_score_is_not_in_score_map() {
        let history = vec![
            LeaderboardRoundHistoryEntry {
                round_num: 1,
                active_players: vec!["Alice".into()],
        ...
        assert!(
            !round_one.contains_key("PendingObserver"),
            "pending observers without scores should not be serialized as score rows"
        );
    }
    ...
    #[tokio::test]
    async fn forced_beauty_timeout_counts_unsubmitted_draft_votes() -> Result<()> {
        let room = test_room();
        {
            let mut state = room.state.write().await;
            for player in ["a", "b", "c", "d"] {
                ...
                setup_connected_member(
                    ...
                    player,
                    &format!("token-{player}"),
                    21_000 + u64::from(player.as_bytes()[0]),
                );
            }
            state.stage = RoomStage::BeautyVoting;
            state.game_mode = GameMode::DixitPlus;
        ...
        room.handle_client_msg(
            "b",
            21_098,
            to_ws(ClientMsg::SetBeautyVoteDraft {
                cards: vec!["ca".into()],
            }),
        )
        .await?;
        ...
        assert!(
            matches!(state.stage, RoomStage::Results),
            "forced beauty timeout should advance after counting draft votes"
        );
        assert_eq!(
            state.player_to_beauty_votes.get("b"),
            Some(&vec!["ca".to_string()]),
            "partial draft beauty votes should become counted beauty votes"
        );
        assert_eq!(
            state.player_to_beauty_votes.get("c"),
            Some(&vec!["cb".to_string(), "ca".to_string()]),
            "complete draft beauty votes should become counted beauty votes"
        );
        assert!(
            !state.player_to_beauty_votes.contains_key("d"),
            "players without submitted or draft beauty votes should still be skipped"
        );
    ...
    #[tokio::test]
    async fn forced_storyteller_voting_scores_draft_votes_but_not_random_fill() -> Result<()> {
        let room = test_room();
        {
            let mut state = room.state.write().await;
            ...
            state.stage = RoomStage::Voting;
            state.votes_per_guesser = 2;
            state.storyteller_success_points = 3;
            state.moderators.insert("host".to_string());
            setup_connected_member(&mut state, "host", "t-host", 31_001);
            setup_connected_member(&mut state, "b", "t-b", 31_002);
            setup_connected_member(&mut state, "c", "t-c", 31_003);
        ...
        room.handle_client_msg(
            ...
            31_003,
            to_ws(ClientMsg::SetVoteDraft {
                cards: vec!["b-card".into(), "c-card".into()],
            }),
        )
        .await?;
        room.handle_client_msg("host", 31_001, to_ws(ClientMsg::ForceCurrentStage {}))
            .await?;

        let state = room.state.read().await;
        assert!(
            matches!(state.stage, RoomStage::Results),
            "force current stage should finish voting with draft votes plus random fill"
        );
        assert_eq!(
            state.storyteller_point_change.get("host").copied(),
            Some(3),
            "draft correct votes should count toward the storyteller outcome"
        );
        assert_eq!(
            state.storyteller_point_change.get("b").copied(),
            Some(4),
            "draft correct votes should score for the drafting guesser and draft decoy votes should count"
        );
        assert_eq!(
            state.storyteller_point_change.get("c").copied(),
            Some(1),
            "manual draft decoy votes should still award decoy points without random-fill influence"
        );
        assert_eq!(
            state.forced_random_vote_cards.get("b").map(Vec::len),
            Some(1),
            "only the random-filled token should be marked forced for a partial draft ballot"
        );
    ...
    async fn forced_players_choose_uses_unsubmitted_draft_cards() -> Result<()> {
        let room = test_room();
        {
            ...
            setup_connected_member(&mut state, "host", "t-host", 32_001);
            setup_connected_member(&mut state, "b", "t-b", 32_002);
        }
        ...
        .await?;
        room.handle_client_msg("host", 32_001, to_ws(ClientMsg::ForceCurrentStage {}))
            .await?;
    ...
    #[tokio::test]
    async fn forced_stella_associate_uses_unsubmitted_draft_order() -> Result<()> {
        let room = test_room();
        {
            let mut state = room.state.write().await;
            state.game_mode = GameMode::Stella;
            ...
            state.stella_selection_max = 5;
            state.stella_clue_word = "spark".into();
            for player in ["host", "b", "c"] {
                add_player(&mut state, player, 0);
            }
            state.moderators.insert("host".to_string());
            setup_connected_member(&mut state, "host", "t-host", 33_001);
            setup_connected_member(&mut state, "c", "t-c", 33_003);
            state
                .stella_player_selections
                .insert("host".into(), vec!["s2".into()]);
        ...
        room.handle_client_msg(
            "c",
            33_003,
            to_ws(ClientMsg::SetStellaSelectionDraft {
                cards: vec!["s3".into(), "s1".into()],
            }),
        )
        .await?;
        room.handle_client_msg("host", 33_001, to_ws(ClientMsg::ForceCurrentStage {}))
            .await?;
        ...
        assert_eq!(
            state.stella_player_selections.get("c"),
            Some(&vec!["s3".to_string(), "s1".to_string()]),
            "draft stella selections should be locked in with queue order preserved"
        );
    ...
    async fn stella_dark_penalty_only_counts_boxes_the_player_scored() -> Result<()> {
        ...
        state.stella_revealed_cards = vec!["shared-early".into()];
